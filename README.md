...
몇 가지 의도적으로 정한 원칙이 있습니다:

- **점수를 합산하지 않습니다.** 처음엔 0–100 "AI Preparedness Index" 같은 가중합산 점수를
  구상했지만, 곰곰이 생각해보니 사용자에게 별 설득력이 없다고 판단해 폐기했습니다. 대신 각 점검
  항목을 **pass/warning/fail/info**로 독립적으로 보여주는 체크리스트 방식을 택했습니다(Lighthouse
  4개 점수만 예외 — 그건 구글 자체 채점이라 숫자를 그대로 보여줍니다).
- **수익 모델은 광고입니다.** 로그인도, 유료 플랜도 없는 무료 도구 + Google 애드센스로 운영할
  계획입니다. 그래서 UI/UX도 "점수를 팔기 위한 도구"가 아니라 "정직하게 진단만 해주는 도구"에
  가깝게 설계하고 있습니다.
