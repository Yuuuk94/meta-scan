...
GitHub Issues 기반으로 **인터뷰 → TDD 개발 → 테스트 → PR**을 처리하는 에이전트 파이프라인
(`tdd-issue-loop` 스킬)도 실제로 구축해서 쓰고 있고, PR이 열린 뒤 실제로 `main`에 배포하는 것까지는
별도 `deploy` 스킬이 맡습니다(`docs/harness/`). 그리고 이 과정 자체가 기술 효율만이 아니라
**학습**을 겸하고 있어서(예: 프론트/백엔드에 일부러 다른 테스트 러너를 도입하는 등), 항상 "가장
효율적인 선택"만 하지는 않는다는 점도 이 저장소의 특징입니다 — 이유는 늘 ADR/케이스 스터디에
남아있습니다.
