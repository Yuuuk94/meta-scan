...
>
> **2026-09-08 수정**: 이슈 #44(root pingApi 콜드스타트 hotfix)를 실제로 처리하면서 세 가지가
> 드러나 반영함(전부 사용자와의 인터뷰로 확정) — (1) 5개 에이전트 전부 브랜치 전략이
> `feat/*`+`dev`로 하드코딩돼 있어서 hotfix를 못 태웠음 → `type:hotfix` 라벨 도입, `dev-interview`가
> 매번 hotfix 여부를 물어 라벨을 붙이고, 나머지 4개 에이전트는 이 라벨을 보고 `hotfix/<n>-*`(off
> `main`) vs `feat/<n>-*`(off `dev`)로 분기 — 아래 결정 표/에이전트별 역할/라벨 체계 절 갱신.
> (2) `SKILL.md`가 백로그(이미 있는 이슈)만 입력으로 상정하고 있어서 대화 중 막 나온 요청은 이슈
> 등록부터 사람이 직접 해야 했음 → `dev-interview`가 이슈 번호 대신 raw 설명을 받으면 스스로
> `gh issue create`부터 하고 이어서 진행하도록 확장. (3) "PR 오픈 후 사람이 직접 서버 켜서
> 테스트"가 매번 리마인드만 하는 문구였는데 실제로 스킬(오케스트레이팅 세션)이 그 자리에서 서버
> 실행+테스트 재실행+코드 리뷰까지 하고 결과를 사람에게 넘기는 걸로 강화 — 아래 "전체 흐름"/
> "스킬 진입점" 절 참고. 머지 자체(및 hotfix의 main→dev 백머지, release 컷)는 여전히 이 스킬 밖 —
> 별도 `deploy` 스킬(`.claude/skills/deploy/SKILL.md`, `docs/harness/deploy-skill.md`)의 몫으로
> 분리함.
>
...
*(2026-09-08 기준 이 다이어그램은 hotfix 분기(`type:hotfix` → `main`/`hotfix/*`)와 PR 오픈 후
스킬의 서버+테스트+코드 리뷰 단계를 아직 그림으로 반영 안 함 — 텍스트 설명(위 결정 표, 아래
"스킬 진입점")이 최신이고 다이어그램 갱신은 후속 작업.)*
...
`type:hotfix` — 2026-09-08 추가. 사람이 미리 붙이는 게 아니라 `dev-interview`가 매번(인터뷰 시작
전) hotfix 여부를 물어서 직접 붙임(위 "브랜치 전략 분기" 행 참고). 없으면(기본) 일반 `feat`/`dev`
경로.
...
0. **(2026-09-08 추가) 사용자가 이슈 번호/백로그 처리 대신 대화 중에 문제/기능을 직접 설명했다면**,
   백로그 조회를 건너뛰고 `dev-interview`에 그 설명을 그대로 넘김 — `dev-interview`가 `gh issue
   create`로 이슈를 스스로 만든 뒤(1번부터는 동일) 진행. 인터뷰 도중 이슈가 여러 개로 쪼개지면
   각각을 아래 2번 큐 규칙(한 번에 하나, 이전 PR 머지 전 다음 이슈 진입 안 함) 그대로 태움 — 별도
   로직 아님.
...
6. 한 이슈가 `status:in-review`(PR 오픈)에 도달하면 스킬(오케스트레이팅 세션)이 그 자리에서
   메인 워킹 디렉토리에 브랜치를 체크아웃해 서버 실행+테스트 재실행+코드 리뷰까지 해서 사람에게
   넘기고(2026-09-08 추가, 예전엔 "사람이 직접 하라"는 리마인드뿐이었음), 그 이슈는 이 스킬
   호출의 dev/qa 처리 대상에서 빠지고 위 4번대로 dev/qa 큐 전체가 이번 호출에서는 거기서 멈춤.
   PR 머지는 항상 사람이 직접 승인하며, 스킬/에이전트가 대신 머지하는 경우는 없음 — 실제 머지
   (및 hotfix의 `main`→`dev` 백머지, release 컷)는 별도 `deploy` 스킬의 몫
