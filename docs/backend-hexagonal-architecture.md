...
> `docs/backend-architecture-survey.md`(7개 패턴 조사) 참고 — 이 문서는 그중 최종 채택된
...
`docs/backend-architecture-survey.md`에서 정리한 대로, 지금 구조(레이어드
...
2. 7개 아키텍처 패턴(NestJS 기본, Clean/Onion, Hexagonal, DDD 모듈러 모놀리스, CQRS, Vertical
   Slice, 마이크로서비스)을 압축 없이 전부 조사 — `docs/backend-architecture-survey.md`.
...
5. **"entities 계층이 얕아 보인다"는 이유로 미리 기각하지 않고**, PRD
   (`docs/meta-scan-plus-prd.md` 3~4장)의 체크리스트 명세를 실제로 읽어 사실관계를 확인 —
...
전체 비교는 `docs/backend-architecture-survey.md` 참고. 최종 좁혀진 두 후보만 요약:
