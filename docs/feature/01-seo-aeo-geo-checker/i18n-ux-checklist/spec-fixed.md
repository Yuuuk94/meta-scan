...
1. Given `crawling`의 DOM 파싱, When `<link rel="alternate" hreflang="...">`를 확인하면,
   Then `hreflang`(존재=pass, 없음=info)을 `checks.i18nUx`에 추가한다
2. Given `crawling`의 DOM 파싱, When `<meta name="viewport">`를 확인하면,
   Then `viewport`(존재=pass, 없음=warning)를 `checks.i18nUx`에 추가한다
3. Given `checks.i18nUx`, When `/scan/:id`를 렌더하면, Then 신규 "국제화/UX" 카드가 2개 항목을
   실데이터 배지로 표시한다
